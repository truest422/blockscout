defmodule Explorer.Account.Notifier.Notify do
  ...
  defp is_watched(%WatchlistAddress{} = address, %{type: type}, direction) do
    case {type, direction} do
      ...
      {"SVC-20", :incoming} -> address.watch_erc_20_input
      {"SVC-20", :outgoing} -> address.watch_erc_20_output
      {"SVC-721", :incoming} -> address.watch_erc_721_input
      {"SVC-721", :outgoing} -> address.watch_erc_721_output
      {"SVC-1155", :incoming} -> address.watch_erc_1155_input
      {"SVC-1155", :outgoing} -> address.watch_erc_1155_output
    end
  end
